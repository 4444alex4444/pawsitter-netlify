...
const CAT_EVENT_LINES: Record<string, string[]> = {
  cat: [
  ...
  vet: [
    ...
    'Хорошо. Здесь явно умеют не паниковать заранее.',
    'Ветконтакт записан. Уважаю запасной план.',
  ],
  walks: [
  ...
  dates: [
    'Даты стоят. Операция «спокойная поездка» началась.',
    'Так. Чемоданы мысленно собраны, кот морально готов осуждать.',
    'План есть. Уже красиво.',
  ],
  final: [
...
function pickLine(list: string[], seed?: string) {
...
function ScenarioHint({ form }: { form: any }) {
  const [catPos, setCatPos] = useState({ x: 18, y: 18 });
  const [bubble, setBubble] = useState(CAT_IDLE_LINES[0]);
  const [eventKey, setEventKey] = useState('');

  const text = useMemo(() => {
    if (form.petType === 'cat') return 'Спокойные визиты, корм, лоток и ощущение, что никто не покушается на священное царство кота.';
    if (form.petType === 'dog') return 'Для собак важны прогулки, контакт и внятный ритм дня. Без скуки и без хаоса.';
    if (form.petType === 'bird') return 'Для птиц важны режим, свежая вода, клетка и человек без резких сюрпризов.';
    if (form.petType === 'rabbit' || form.petType === 'hamster') return 'Маленьким пушистым нужны тишина, аккуратность, корм и уважение к их маленькому внутреннему драмтеатру.';
    return 'Форма подстроится под питомца и сценарий. Без одинаковой скучной анкеты для всех.';
  }, [form.petType]);

  useEffect(() => {
    let nextKey = '';
    let pool = CAT_IDLE_LINES;

    if (form.petType && !form.foodType && !form.sleep && !(form.meds?.length) && !form.vetName) {
      nextKey = `pet:${form.petType}`;
      pool = CAT_EVENT_LINES[form.petType] || CAT_IDLE_LINES;
    }
    if (form.foodType || form.foodBrand) {
      nextKey = `food:${form.foodType}:${form.foodBrand}`;
      pool = CAT_EVENT_LINES.food;
    }
    if (form.sleep === 'couch' || form.sleep === 'bed') {
      nextKey = `sleep:${form.sleep}`;
      pool = CAT_EVENT_LINES.couch;
    }
    if ((form.meds && form.meds.length) || form.noMeds === false && form.chronic) {
      nextKey = `meds:${(form.meds || []).length}:${form.chronic || ''}`;
      pool = CAT_EVENT_LINES.meds;
    }
    if (form.vetName || form.vetPhone) {
      nextKey = `vet:${form.vetName || ''}:${form.vetPhone || ''}`;
      pool = CAT_EVENT_LINES.vet;
    }
    if (form.petType === 'dog' && form.walks >= 3) {
      nextKey = `walks:${form.walks}`;
      pool = CAT_EVENT_LINES.walks;
    }
    if (form.dateFrom && form.dateTo) {
      nextKey = `dates:${form.dateFrom}:${form.dateTo}`;
      pool = CAT_EVENT_LINES.dates;
    }
    if (form.ownerName && form.ownerPhone && form.emergPhone) {
      nextKey = `final:${form.ownerName}:${form.ownerPhone}`;
      pool = CAT_EVENT_LINES.final;
    }

    if (nextKey && nextKey !== eventKey) {
      setEventKey(nextKey);
      setBubble(pickLine(pool, nextKey));
      return;
    }

    const timer = window.setInterval(() => {
      setBubble(pickLine(CAT_IDLE_LINES));
    }, 5600);
    return () => window.clearInterval(timer);
  }, [form, eventKey]);

  function moveCat() {
    const x = Math.max(8, Math.min(78, Math.random() * 70 + 8));
    const y = Math.max(8, Math.min(72, Math.random() * 58 + 8));
    setCatPos({ x, y });
    setBubble(pickLine(CAT_HOVER_LINES));
  }

  return (
    <>
      <Card className="side-card">
        <h3>Ваш питомец не один</h3>
        <p>{text}</p>
        <div className="fact-list">
          <div className="fact"><strong>Что делает ситтер</strong> Кормит, наводит порядок, играет, выгуливает и присылает фото без театра «а всё ли там нормально?».</div>
          <div className="fact"><strong>Как всё проходит</strong> Вы оставляете заявку → мы подбираем ситтера → вы получаете понятный план и спокойную голову.</div>
          <div className="fact"><strong>Когда это особенно удобно</strong> Отпуск, командировка, длинный рабочий день, срочные дела или просто питомец, который не любит переезды.</div>
        </div>
      </Card>
      <div className="mascot" style={{ overflow: 'hidden', minHeight: 230 }}>
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(circle at 18% 18%, rgba(255,255,255,.95), rgba(255,255,255,0) 42%)' }} />
        <div
          className="pet cat"
          onMouseEnter={moveCat}
        ...
        >🐈</div>
        <div className="bubble" style={{ marginTop: 126, position: 'relative', zIndex: 1 }}>{bubble}</div>
        <div style={{ marginTop: 14, fontSize: 12, color: 'var(--muted)', lineHeight: 1.65 }}>Сегодня главный комментатор формы — кот. Он драматичный, местами полезный и подозрительно хорошо разбирается в диванах.</div>
      </div>
    </>
  );
}
    ...
    <div className="success-wrap">
      <Card className="success-card">
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ margin: '0 auto 16px', color: 'var(--muted)', maxWidth: 560, lineHeight: 1.8 }}>Заявка улетела куда надо. Дальше мы подберём ситтера и свяжемся с вами без драм, пропаж и внезапного цифрового цирка.</p>
        </div>
        <div className="summary">
          <div className="item"><strong>Питомец</strong>{form.petName || '—'} · {PET_LABEL[form.petType] || '—'}</div>
